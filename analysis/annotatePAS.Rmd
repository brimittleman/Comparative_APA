...
```{r}
BothMean_5_out=BothMean_5 %>% select(PAS,disc, gene, loc,chr, start, end,Chimp, Human)
write.table(BothMean_5_out, "../data/Peaks_5perc/Peaks_5perc_either_bothUsage.txt", row.names = F, col.names = T, quote = F)

BothMean_5_out_noUN=BothMean_5 %>% select(PAS,disc, gene, loc,chr, start, end,Chimp, Human) %>% filter(!grepl("Un",chr))
