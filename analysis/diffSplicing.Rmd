...
Bams=as.data.frame(colnames(FilteredClusterBoth)) %>% mutate(Species=ifelse(grepl("H",colnames(FilteredClusterBoth)), "Human", "Chimp")) %>% slice(2:n())

#mkdir ../data/DiffSplice 
write.table(Bams, "../data/DiffSplice/groups_file.txt", col.names = F, row.names = F, quote = F, sep="\t" )


write.table(FilteredClusterBoth, "../data/DiffSplice/BothSpec_perind.counts", col.names = T, row.names = F, quote = F, sep="\t" )
