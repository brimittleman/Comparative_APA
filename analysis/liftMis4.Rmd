...
I will run my liftover pipeline for the misprime4 data. I am using the code from the original [liftover](analysis/liftoverPAS.html)  


Do all of the bash from the misprime4 directory, R script comes from the comparative APA dir. Copy necessary code and fix direction to chain files.  
```{r}
library(tidyverse)
...
cp ../../Comparative_APA/code/primaryLift.sh . #new dir  ../../Comparative_APA/data/chainFiles/
cp ../../Comparative_APA/code/reverseLift.sh . #new dir  ../../Comparative_APA/data/chainFiles/
...
```{r}
unliftedH=read.table("../../Misprime4/data/primaryLift/human_APApeaks_primarylift2Chimp_UNLIFTED.bed",stringsAsFactors = F) %>% nrow()
unliftedC=read.table("../../Misprime4/data/primaryLift/chimp_APApeaks_primarylift2Human_UNLIFTED.bed",stringsAsFactors = F) %>% nrow()

liftedH=read.table("../../Misprime4/data/primaryLift/human_APApeaks_primarylift2Chimp.bed",stringsAsFactors = F) %>% nrow()
liftedC=read.table("../../Misprime4/data/primaryLift/chimp_APApeaks_primarylift2Human.bed",stringsAsFactors = F) %>% nrow()
...
```{r}
re_unliftedH=read.table("../../Misprime4/data/reverseLift/human_APApeaks_primarylift2Human_rev2Human_UNLIFTED.bed",stringsAsFactors = F) %>% nrow()
re_unliftedC=read.table("../../Misprime4/data/reverseLift/chimp_APApeaks_primarylift2Human_rev2Chimp_UNLIFTED.bed",stringsAsFactors = F) %>% nrow()

re_liftedH=read.table("../../Misprime4/data/reverseLift/human_APApeaks_primarylift2Chimp_rev2Human.bed",stringsAsFactors = F) %>% nrow()
re_liftedC=read.table("../../Misprime4/data/reverseLift/chimp_APApeaks_primarylift2Human_rev2Chimp.bed",stringsAsFactors = F) %>% nrow()
...
```{r}
Human_recLift=read.table("../../Misprime4/data/cleanPeaks_lifted/Human_PASregions.bed",stringsAsFactors = F, col.names = c("chr", "start","end", "name", "score", "strand")) 
Chimp_recLift=read.table("../../Misprime4/data/cleanPeaks_lifted/Chimp_PASregions.bed",stringsAsFactors = F,col.names = c("chr", "start","end", "name", "score", "strand"))
...
```
```{bash,eval=F}
cp ../../Comparative_APA/code/recLiftchim2human.sh . # ../../Comparative_APA/data/chainFiles/
...
cp ../../Comparative_APA/code/intersectLiftedPAS.sh .
...
```{r}
OverlapBoth=read.table("../../Misprime4/data/cleanPeaks_lifted/PASregions_identifiedboth.txt", col.names = c("Hchr", "Hstart","Hend", "Hname", "Hscore", "Hstrand", "Cchr", "Cstart", "Cend", "Cname", "Cscore", "Cstrand", "overlap"),stringsAsFactors = F) %>% mutate(meanScore=(Hscore+Cscore)/2, name=paste("Both", Hname, sep=":"), Bothname=paste(Hname, Cname, sep=":"))

overlap2= OverlapBoth %>% group_by(name) %>% filter(n()>1) %>%  mutate(id = row_number()) %>% filter(id==2)

OverlapBoth_format=read.table("../../Misprime4/data/cleanPeaks_lifted/PASregions_identifiedboth.txt", col.names = c("Hchr", "Hstart","Hend", "Hname", "Hscore", "Hstrand", "Cchr", "Cstart", "Cend", "Cname", "Cscore", "Cstrand", "overlap"),stringsAsFactors = F) %>% mutate(meanScore=(Hscore+Cscore)/2, name=paste("Both", Hname, sep=":"),Bothname=paste(Hname, Cname, sep=":")) %>% filter(!Bothname %in% overlap2$Bothname) %>% dplyr::select(Hchr,Hstart,Hend,name,meanScore,Hstrand) 



HumanSpec=read.table("../../Misprime4/data/cleanPeaks_lifted/PASregions_identifiedHuman.txt", col.names = c("Hchr", "Hstart","Hend", "Hname", "meanScore", 'Hstrand'),stringsAsFactors = F) %>% mutate(name=paste("Human", Hname, sep=":")) %>% dplyr::select(Hchr, Hstart,Hend, name, meanScore, Hstrand) 


ChimpSpec=read.table("../../Misprime4/data/cleanPeaks_lifted/PASregions_identifiedChimp.txt", col.names = c("Hchr", "Hstart","Hend", "Cname", "meanScore", 'Hstrand'),stringsAsFactors = F) %>% mutate(name=paste("Chimp", Cname, sep=":")) %>% dplyr::select(Hchr, Hstart,Hend, name, meanScore, Hstrand)
...
write.table(AllPAS_use,  "../../Misprime4/data/cleanPeaks_lifted/AllPAS_postLift.bed", col.names = F, row.names = F, quote = F, sep = "\t")
