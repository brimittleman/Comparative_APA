...
I am looking at PAS at .1 or greater in either species. 
```{r}
...
```
```{r}
ggplot(PAS_m,aes(x=nPAS, by=species, fill=species)) + geom_density(stat="count",alpha=.5) +  scale_fill_brewer(palette = "Dark2") + labs(title="Distribution for number of PAS >= 10%", x="Number of PAS",y="Genes")
```
