...
#nuclear apa  
NucAPA=read.table("../data/DiffIso_Nuclear/TN_diff_isoform_allChrom.txt_significance.txt",sep="\t" ,col.names = c('status','loglr','df','p','cluster','p.adjust'),stringsAsFactors = F) %>% filter(status=="Success") 
NucAPA$p.adjust=as.numeric(as.character(NucAPA$p.adjust))
NucAPA_genes=NucAPA %>% filter(p.adjust<.05) %>% separate(cluster, into=c("chrm", "gene"),sep=":")
#totalAPA  
totAPA=read.table("../data/DiffIso_Total/TN_diff_isoform_allChrom.txt_significance.txt",sep="\t" ,col.names = c('status','loglr','df','p','cluster','p.adjust'),stringsAsFactors = F) %>% filter(status=="Success") 
totAPA$p.adjust=as.numeric(as.character(totAPA$p.adjust))
totAPA_genes=totAPA %>% filter(p.adjust<.05) %>% separate(cluster, into=c("chrm", "gene"),sep=":")
#ds
...
listInput <- list(DE=DEgenes$Gene.name, DS=DSgenes$DS, DapaNuc=NucAPA_genes$gene, DapaTot=totAPA_genes$gene, DT=Translation$Gene, DP=Protein$gene.symbol)
...
```{r}
listInput_nucOnly <- list(DE=DEgenes$Gene.name, DS=DSgenes$DS, DAPA=NucAPA_genes$gene, DT=Translation$Gene, DP=Protein$gene.symbol)
