---
title: "Intronic Location for Shared and Different Dominant PAS Double Filter"
...
```{r}
HumanIntronicChimpUTR=read.table("../data/DominantPAS_DF/Nuclear_HumanIntronicChimpUTR.txt",header = T, stringsAsFactors = F)%>% dplyr::select(gene, HumanPAS, Human)
SameDomIntron=read.table("../data/DominantPAS_DF/SameDominantIntronic.txt",header = T, stringsAsFactors = F)%>% dplyr::select(gene, HumanPAS,Human)
```

```{r}
HumanMetadata= read.table("../data/PAS_doubleFilter/PAS_10perc_either_HumanCoord_BothUsage_meta_doubleFilter.txt",header = T,stringsAsFactors = F) %>% rename("HumanPAS"=PAS) %>% dplyr::select(-Chimp)
ChimpMetadata= read.table("../data/PAS_doubleFilter/PAS_10perc_either_HumanCoord_BothUsage_meta_doubleFilter.txt",header = T,stringsAsFactors = F) %>% rename("ChimpPAS"=PAS) %>% dplyr::select(-Human)
```
...
```{r}
HumanPAS_samedom=HumanMetadata %>% inner_join(SameDomIntron, by="HumanPAS") %>% mutate(GenePAS=paste(gene.x, HumanPAS, sep="_")) %>% dplyr::select(chr, start, end, GenePAS, Human.x,strandFix)

HumanPAS_diffDom=HumanMetadata %>% inner_join(HumanIntronicChimpUTR, by="HumanPAS")%>% mutate(GenePAS=paste(gene.x, HumanPAS, sep="_")) %>% dplyr::select(chr, start, end, GenePAS, Human.x,strandFix)
```

I can write these out as bed files. 

```{r}
write.table(HumanPAS_samedom, "../data/DominantPAS_DF/SameDominantPAS_intronic.bed", quote = F, row.names = F, col.names = F,sep = "\t")
write.table(HumanPAS_diffDom, "../data/DominantPAS_DF/DifferentDominantPAS_intronic.bed", quote = F, row.names = F, col.names = F,sep = "\t")
```
...
```{bash,eval=F}
sbatch FindIntronForDomPAS_DF.sh
```
...
```{r}
SameDomIntron=read.table("../data/DominantPAS_DF/SameDominantPAS_intronic_mapped2Intron.txt", stringsAsFactors = F, col.names = c("PASchr", "PASstart", "PASend", "PASname", "PASusage", "PASstrand", "Intronchr", "IntronStart", "IntronEnd", "IntronName", "IntronScore", "IntronStrand"))
...
DiffDomIntron=read.table("../data/DominantPAS_DF/DifferentDominantPAS_intronic_mapped2Intron.txt", stringsAsFactors = F, col.names = c("PASchr", "PASstart", "PASend", "PASname", "PASusage", "PASstrand", "Intronchr", "IntronStart", "IntronEnd", "IntronName", "IntronScore", "IntronStrand"))
