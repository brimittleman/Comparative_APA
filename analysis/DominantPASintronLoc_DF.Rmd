...
```{r}
HumanMetadata= read.table("../data/PAS_doubleFilter/PAS_5perc_either_HumanCoord_BothUsage_meta_doubleFilter.txt",header = T,stringsAsFactors = F) %>% rename("HumanPAS"=PAS) %>% dplyr::select(-Chimp)
ChimpMetadata= read.table("../data/PAS_doubleFilter/PAS_5perc_either_HumanCoord_BothUsage_meta_doubleFilter.txt",header = T,stringsAsFactors = F) %>% rename("ChimpPAS"=PAS) %>% dplyr::select(-Human)
```
