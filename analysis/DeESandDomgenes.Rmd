...
```{r}
DeandAPA= DiffIso %>% inner_join(DE, by="gene") %>% filter(gene %in% BothRes_40$gene)

ggplot(DeandAPA,aes(y=deltaPAU, x=logFC,col=loc)) + geom_point(alpha=.3) + geom_smooth(aes(col=loc), method="lm") + labs(title="Intronic and 3' UTR APA v DE\ gene with dominance") + scale_color_brewer(palette = "Dark2")+ stat_cor(aes(color = loc), label.x = 3)
```
###Significant:  
...
ggplot(DeandAPA_domPAS,aes(y=deltaPAU, x=logFC,col=loc)) + geom_point(alpha=.3) + geom_smooth(aes(col=loc), method="lm") + labs(title="Intronic and 3' UTR APA v DE\ Dominant PAS") + scale_color_brewer(palette = "Dark2")+ stat_cor(aes(color = loc), label.x = 3)
```
###Significant:  

```{r}
DeandAPA_domPAS_sigAPA= DeandAPA_domPAS %>% filter(SigPAU2=="Yes")
ggplot(DeandAPA_domPAS_sigAPA,aes(y=deltaPAU, x=logFC, col=loc)) + geom_point(alpha=.3) + geom_smooth(method="lm")+ labs(title="Significant Intronic and 3' UTR APA v DE \n Dom PAS")+ scale_color_brewer(palette = "Dark2")+ stat_cor(aes(color = loc), label.x = 3)
```
...
RiboandAPAdom=DiffIso %>% inner_join(Ribo, by="gene")%>% filter(gene %in% BothRes_40$gene)%>% filter(PAS %in% AllDomPAS)

ggplot(RiboandAPAdom,aes(y=deltaPAU, x=HvC.beta, col=loc)) + geom_point(alpha=.3) + geom_smooth(method="lm") + labs(title="APA v Ribosome Occupany")+ scale_color_brewer(palette = "Dark2")+ stat_cor(aes(color = loc), label.x = 3)
```
###Significant:  

```{r}
RiboandAPAdom_sigAPA= RiboandAPAdom %>% filter(SigPAU2=="Yes")
ggplot(RiboandAPAdom_sigAPA,aes(y=deltaPAU, x=HvC.beta, col=loc)) + geom_point(alpha=.3) + geom_smooth(method="lm")+ labs(title="Significant Intronic and 3' UTR APA v DE \n Dom PAS sig apa")+ scale_color_brewer(palette = "Dark2")+ stat_cor(aes(color = loc), label.x = 3)
```

```{r}
RiboandAPAdom_sigAPAandR= RiboandAPAdom %>% filter(SigPAU2=="Yes",  HvC.FDR<.05)
ggplot(RiboandAPAdom_sigAPAandR,aes(y=deltaPAU, x=HvC.beta, col=loc)) + geom_point(alpha=.3) + geom_smooth(method="lm")+ labs(title="Significant Intronic and 3' UTR APA v DR \n Dom PAS sig both")+ scale_color_brewer(palette = "Dark2")+ stat_cor(aes(color = loc), label.x = 3)
```
Not enough power for this.  
