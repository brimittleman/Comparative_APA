...
ggplot(Both2PerUniq_g, aes(x=loc, y=Usage, by=Species, fill=Species)) + geom_boxplot() + facet_grid(~MultiMap)
```
...
```{r}
x= nrow(allPAS_annoDomandMM %>% filter(DomHuman =="Yes",MMuniq_human=="Yes" ))
...
```{r}
x= nrow(allPAS_annoDomandMM %>% filter(DomChimp =="Yes",MMuniq_chimp=="Yes" ))
...
```{r}
ChimpDomandMM=allPAS_annoDomandMM %>% filter(DomChimp =="Yes",MMuniq_chimp=="Yes") %>% mutate(Species="Chimp")
HumanDomandMM=allPAS_annoDomandMM %>% filter(DomHuman =="Yes",MMuniq_human=="Yes")%>% mutate(Species="Human")


bothDomandMM= ChimpDomandMM %>% bind_rows(HumanDomandMM)


ggplot(bothDomandMM,aes(x=loc, fill=Species,by=Species)) + geom_bar(stat="count", position = "dodge") + scale_fill_brewer(palette = "Dark2") + labs(x="", y="Number of PAS", title="Location of Multimapping PAS condition on dominance")
```
