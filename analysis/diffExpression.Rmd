...
ggplot(readProp, aes(x=Line,y=Proportion, fill=Category)) + geom_bar(stat="identity") + scale_fill_manual(values = useCOl, name="", labels = c("Unmapped", "Not assigned to Ortho Exon", "Assigned Ortho Exon"))+theme(axis.text.x = element_text( hjust = 0,vjust = 1, size = 12, angle = 90),legend.position ="bottom",panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black"),axis.text.y = element_text(size=12),plot.title = element_text(hjust = 0.5)) + labs(y="Reads", title="Human and chimp read proportions") 
...
```{r,include=F, eval=F}
pdf("../output/supplement/Fig3-figSup4A.pdf", height=4, width=4,useKerning=F)
  plot_scores(pca_genes, scores, 1, 1+1, col.v)
dev.off()
```
...
```
```{r,include=F, eval=F}
pdf("../output/supplement/Fig3-figSup4B.pdf", height=8, width=8,useKerning=F)
heatmap.2(as.matrix(pc_cov_cor[Tech_fac,PCs]),col=brewer.pal(4, "Greens"), trace="none",
          Rowv=FALSE, Colv=FALSE, key=T, main="Cor. PCs & tech factors", dendrogram="none",
          key.title=NA, cexRow=0.9, cexCol=0.9)
dev.off()
...
```{r}
top.table2=top.table %>% mutate(Species=ifelse(logFC >0 & adj.P.Val<.05, "Chimp", ifelse(logFC < 0 & adj.P.Val< .05, "Human", "Neither")))
  
  
```

```{r, DEplot}
ggplot(top.table2, aes(x=logFC, y= -log10(adj.P.Val))) + geom_point(aes(col=Species), alpha=.3) + scale_color_brewer(palette = "Dark2") + labs(title="Differential Expression") +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black"), legend.position = "bottom", plot.title = element_text(hjust = 0.5))
```
