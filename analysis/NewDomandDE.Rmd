...
```{r}
ResDFSameAPA=as.data.frame(cbind(set,pval=pvalBothSideapa,enrich=EnrichSameAPA, actual=actualSameapa,expected=ExpectedSameAPA)) %>% mutate(dAPA="Yes")
ResDFSameNo=as.data.frame(cbind(set,pval=pvalBothSideno,enrich=EnrichSameNo, actual=actualSameNo,expected=ExpectedSameNo))%>% mutate(dAPA="No")
...
plot as a barplot to see with order:  
