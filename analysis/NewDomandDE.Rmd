...
ggplot(PropDE, aes(x=set,y=Prop,fill=set)) + geom_bar(stat="identity",alpha=.5)+ labs(title="Proportion of DE genes with different dominant PAS", y="Proportion of DE genes")+ scale_fill_brewer(palette = "RdYlBu") + theme(legend.position = "none")+geom_text(aes(label=OverlapDE), position=position_dodge(width=0.9), vjust=1.5)
```
