...
cp ../../Comparative_APA/code/LiftOrthoPAS2chimp.sh . #new dir  ../../Comparative_APA/data/chainFiles/
...
```{r}
humanPAS=read.table("../../Misprime4/Human/data/CleanLiftedPeaks_FC/ALLPAS_postLift_LocParsed_Human_fixed.fc", header=T, stringsAsFactors = F) %>% 
...
chimpPAS=read.table("../../Misprime4/Chimp/data/CleanLiftedPeaks_FC/ALLPAS_postLift_LocParsed_Chimp_fixed.fc", header=T, stringsAsFactors = F) %>% 
  separate(Geneid, into=c("disc","PAS","chrom", "start","end","strand","geneid"), sep=":") %>%
  separate(geneid,into=c("gene","loc"),sep="_") %>%
  dplyr::select(gene,contains("_N")) %>%
  gather(key="ind", value="count", -gene) %>% 
...
write.table(GenesCutoffDF,"../../Misprime4/data/OverlapBenchmark/genesPassingCuttoff.txt", col.names = T, row.names = F,quote = F)
```
...
```{r}
HumanAnno=read.table("../../Misprime4/Human/data/phenotype/ALLPAS_postLift_LocParsed_Human_Pheno.txt", header = T, stringsAsFactors = F) %>% tidyr::separate(chrom, sep = ":", into = c("chr", "start", "end", "id")) %>% tidyr::separate(id, sep="_", into=c("gene", "strand", "peak"))  %>% separate(peak,into=c("loc", "disc","PAS"), sep="-")
IndH=colnames(HumanAnno)[9:ncol(HumanAnno)]

HumanUsage=read.table("../../Misprime4/Human/data/phenotype/ALLPAS_postLift_LocParsed_Human_Pheno_countOnlyNumeric.txt", col.names = IndH)
...
```{r}
ChimpAnno=read.table("../../Misprime4/Chimp/data/phenotype/ALLPAS_postLift_LocParsed_Chimp_Pheno.txt", header = T, stringsAsFactors = F) %>% tidyr::separate(chrom, sep = ":", into = c("chr", "start", "end", "id")) %>% tidyr::separate(id, sep="_", into=c("gene", "strand", "peak"))  %>% separate(peak,into=c("loc", "disc","PAS"), sep="-")
IndC=colnames(ChimpAnno)[9:ncol(ChimpAnno)]

ChimpUsage=read.table("../../Misprime4//Chimp/data/phenotype/ALLPAS_postLift_LocParsed_Chimp_Pheno_countOnlyNumeric.txt", col.names = IndC)
...
write.table(BothMean_5_out, "../../Misprime4/data/Peaks_5perc/Peaks_5perc_either_bothUsage.txt", row.names = F, col.names = T, quote = F)

BothMean_5_out_noUN=BothMean_5 %>% dplyr::select(PAS,disc, gene, loc,chr, start, end,Chimp, Human) %>% filter(!grepl("Un",chr))

write.table(BothMean_5_out_noUN, "../../Misprime4/data/Peaks_5perc/Peaks_5perc_either_bothUsage_noUnchr.txt", row.names = F, col.names = T, quote = F)
#write bed with human coord for igv
BothMean_5_bed=BothMean_5 %>% dplyr::select(chr, start, end, PAS, Human, strand)
write.table(BothMean_5_bed,"../../Misprime4/data/Peaks_5perc/Peaks_5perc_either_HumanCoordHummanUsage.bed", row.names = F, col.names = T, quote = F)
...
write.table(ChimpUsage_anno_5perc, "../../Misprime4/data/Pheno_5perc/Chimp_Pheno_5perc.txt", row.names = F, col.names = T, quote = F)

HumaUsage_anno_5perc= HumanUsage_anno %>% filter(PAS %in% BothMean_5$PAS)

write.table(HumaUsage_anno_5perc, "../../Misprime4/data/Pheno_5perc/Human_Pheno_5perc.txt", row.names = F, col.names = T, quote = F)
```
