...
SigSep=ggplot(DeandAPA_topPASsigAPAandE,aes(y=deltaPAU, x=logFC,col=loc)) + geom_point(alpha=.3) + geom_smooth(method="lm")+ labs(title="Significant APA, Top PAS  v Significant DE") +  scale_color_brewer(palette = "Set1")+ stat_cor(aes(color = loc), label.x = 1)+theme_classic(base_size = 12) 
...
sigapasigribosep=ggplot(RiboandAPA_topPASsigAPAandR,aes(y=deltaPAU, x=HvC.beta,col=loc)) + geom_point(alpha=.3) + geom_smooth(method="lm")+ labs(title="Significant  APA, top PAS  v Significant Ribosome Occupany") + scale_color_brewer(palette = "Set1")+ stat_cor(aes(color = loc), label.x = 1)+theme_classic(base_size = 12) 
