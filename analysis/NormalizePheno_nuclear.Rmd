...
```{r}
ggplot(pcandExM, aes(x=PC, fill=PVE, y=Experiment))+ geom_tile()+  labs(title="Proportion of variation explained in PCs by Experimental Faction \n Nuclear", y="") + scale_fill_distiller(palette = "Blues", direction=1)
```



```{r}
ggplot(top5PC,aes(x=Species,y=Concentration)) + geom_boxplot(alpha=.5)+ geom_jitter(aes(col=Cycles)) + stat_compare_means(method="t.test")  + labs(title="Nuclear Fraction") 
