...
Write these out. I want to have the PAS and the 100 bases upstream of the PAS only.
```{r}
AllPAS_together=as.data.frame(rbind(Inboth,HumanOnly ,ChimpOnly))  %>% select(chr, start,end, newName, score, strand)
```

```{r}
write.table(AllPAS_together, "../data/cleanPeaks_lifted/AllPAS_postLift.bed", quote = F, col.names = F, row.names = F, sep="\t")
```
