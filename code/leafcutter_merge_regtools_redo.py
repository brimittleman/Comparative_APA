...
def pool_junc_reads(flist, options):
    ...
    for (chrom,strand) in by_chrom:
        read_ks = [k for k,v in by_chrom[(chrom,strand)].items() if v >= 3] # a junction must have at least 3 reads
        read_ks.sort()
        ...
        for cl in clu:
            if len(cl) > 1: # if cluster has more than one intron  
                ...
                for interval, count in [(x, by_chrom[(chrom,strand)][x]) for x in cl]:
    ...
    for libN in merges:
        ...
        for lib in merges[libN]:
            ...
            for ln in F:
                ...
                chrom = "chr"+chrom.replace("chr","")+":"+strand
                Aoff, Boff = blockSize.split(",")
                A, B = int(A)+int(Aoff), int(B)-int(Boff)+1
                if chrom not in by_chrom:
                    by_chrom[chrom] = {}
                intron = (A,B)#(int(start), int(end)+1)
        ...
        for clu in cluExons:
            ...
            for exon in ks:
                ...
                chrom, start, end = exon
                start, end = int(start), int(end)
                chromName, strand = chrom.split("_")
                if chrom not in by_chrom:
                    buf.append("%s:%d:%d:clu_%d_%s 0/%d\n"%(chromName,start, end,clu, strand,tot))
                elif (start,end) in by_chrom[chrom]:                
                    buf.append("%s:%d:%d:clu_%d_%s %d/%d\n"%(chromName,start, end, clu,strand, by_chrom[chrom][(start,end)], tot))
                else:
                    buf.append("%s:%d:%d:clu_%d_%s 0/%d\n"%(chromName,start, end,clu,strand, tot))
...
if __name__ == "__main__":
    ...
    if options.juncfiles == None:
        ...
        exit(0)
    if options.cluster == None:
        sys.stderr.write("Error: no refined cluster -c file provided...\n")
        exit(0)
