...
try: 
    ...
    sys.stderr.write("Usage: python merge_leafcutter_cluster.py combined_refined refined_clusters_1 ...\n")
    exit()
    ...
    for (chrom,strand) in by_chrom:
        ...
        for cl in clu:
            if len(cl) > 1: # if cluster has more than one intron  
                ...
                for fclu in refine_clusters(cluster):
                    fout.write(" ".join(["%s_%s"%(chrom,strand)]+ ["%s:%d"%("%s:%s"%x[0],x[1]) for x in fclu])+'\n')

    fout.close()

def refine_clusters(clu):
    if True:
        Ncl = 0
        for cl in refine_linked(clu):
            rc = refine_cluster(cl)
            if len(rc) > 0:
                for clu in rc:
                    #buf = '%s ' % chrom
...
def refine_cluster(clu, cutoff=0.01, readcutoff=20):
    ...
    for inter, count in clu:
        totN += count
    for inter, count in clu:
        if ((count/float(totN)) >= cutoff and (count >= 10)) or (count >= readcutoff):
